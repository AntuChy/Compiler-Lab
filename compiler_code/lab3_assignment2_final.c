...
void main(){
	...
	LL1 * table_LL1 = NULL;
	printf(">>> E' as A, T' as B, ID as i, N as & <<<\n");
	printf(">>>   Enter .(dot) when finish to write grammar   <<<\n");
	printf("enter modified grammar : \n");
	saisie(&prod);
	printf("Modified grammar is:");
	printf("\n");
	printRules(prod);
	prod_with_no_rec = removeRecursion(prod);
	printf("After remove Recursion: ");
	printf("\n");
	...
	while(1){
		...
		switch (verify){
			case 1:
				printf("Result: The word %s is accepted \n",text);

				break;
			case 0:
				printf("Result: The word %s is rejected  \n",text);
